...
- **直接看结果**：BF16 输出
  `[31880, 31868, 31928, 31869, 31825, 31871, 31744]`，FP8 输出
  `[31880, 31868, 31924, 31864, 31829, 31871, 31744]`。
- **7 个动作里有 4 个完全没变**：`Δx`、`Δy`、`yaw` 和 `gripper` 的 FP8 token 与 BF16 一模一样。也就是说，在这个样本中，FP8 没有改变水平移动、偏航旋转和夹爪开合决策。
- **另外 3 个动作有小幅变化**：`Δz` 相差 4 个档位，`roll` 相差 5 个档位，`pitch` 相差 4 个档位。最大偏差是 5 个档位；相对于 256 个动作 token，约占整个离散范围的 2%。
- **输出格式正常**：两条路径生成的所有 token 都位于合法动作区间 `[31744, 31999]`。FP8 没有生成越界 token，也没有出现无法解码的异常输出。

#### ③ 这组结果说明什么

**直白地说：对这一个测试样本，FP8 给出的动作与 BF16 很接近，但不是完全相同。** 判据：

1. **完全相同的部分**：7 维中有 4 维 token 不变，包括 `gripper`，因此这个样本的抓/放判断没有受到量化影响。
2. **发生变化的部分**：`Δz`、`roll`、`pitch` 分别移动了 4、5、4 个离散档位。变化不大，但解码后的实际物理误差仍取决于各维度的 `q01/q99` 范围，不能只用 token 差直接判断控制影响。
3. **没有输出异常**：所有 token 都合法，说明 FP8 推理链路和动作解码链路工作正常；这里看到的是数值精度变化，不是模型输出损坏。
4. **误差来源可以定位**：`--mode golden-emb`（BF16 vision embedding + FP8 LLM）与 `--mode action`（FP8 vision + FP8 LLM）生成了完全相同的 token。对这个样本而言，vision 的 FP8 量化没有改变最终动作 token，差异来自 FP8 LLM 路径。

因此，本样本可以支持的结论是：**FP8 推理链路输出正常，动作 token 与 BF16 接近，最大相差 5 个离散档位。** 它不能单独证明 FP8 在完整任务上的成功率与 BF16 相同；这一点需要多图像统计和 LIBERO rollout 结果来验证。

> 物理量级的换算（token→物理动作值）见 `common.py:148` 的 `decode_action_tokens`：每个 token 经 `bin_idx → bin_centers → 反归一化(q01/q99)` 三步得到物理量。token 差表示相隔多少个离散档位；由于每个动作维度的反归一化范围不同，同样相差 4 个 token，换算到位移或旋转后的物理误差也可能不同。
...
1. **先看整体结果**：按当前分段计时口径，推理时延从 BF16 的 `466.24 ms` 降到 FP8 的 `236.70 ms`，约加速 `1.97×`。换算成模型推理频率，是从 `2.14 Hz` 提升到 `4.22 Hz`。

   这说明模型侧已经超过 `3 Hz`，但 `4.22 Hz` 是根据模型推理时延计算的理论频率，不包含图像采集、仿真或机器人执行、通信等控制环节。因此，更准确的说法是“模型推理达到 3 Hz 以上”，是否满足整套系统的实时控制要求还需要看完整控制循环。

2. **Prefill 为什么能加速 `1.92×`**：Prefill 会一次处理整段输入，包括视觉 token 和文本 token。它的矩阵乘法规模比 Decode 大，GPU 的 TensorCore 利用率更高。换成 FP8 后，一方面每个权重占用的字节更少，需要搬运的数据减少；另一方面，大矩阵也能更充分利用 FP8 TensorCore 的计算能力。

   实测从 `103.56 ms` 降到 `54.01 ms`。加速没有超过 2 倍，是因为 Prefill 中仍包含未完全量化的层、非矩阵乘法算子、engine 调度等不能按 FP8 算力同比缩短的部分。

3. **Decode 为什么接近 `2×`**：Decode 每一步只生成 1 个 token。此时矩阵乘法规模很小，TensorCore 通常吃不满，主要时间花在反复读取模型权重，而不是做计算。

   FP8 将权重字节数大致减半，所以每步 Decode 从约 `60.4 ms` 降到 `30.45 ms`，6 步合计从 `362.68 ms` 降到 `182.69 ms`，加速 `1.99×`。时延几乎跟随权重数据量减半，说明 Decode 仍然是访存受限，而不是算力受限。

4. **Prefill 和 Decode 谁加速更多**：按当前主文档的数据，Decode 的 `1.99×` 略高于 Prefill 的 `1.92×`，两者都接近 2 倍。因此不应再表述为“Prefill 加速最明显”；更准确的结论是：**Prefill 同时受益于计算和访存优化，Decode 的收益则主要来自权重读取量减半。**
