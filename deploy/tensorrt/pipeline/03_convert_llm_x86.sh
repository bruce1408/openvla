#!/bin/bash
# OpenVLA Llama → Edge-LLM ONNX 量化+导出（x86 GPU 主机执行）
#
# 用法:
#   bash 03_convert_llm_x86.sh              # 只导出 BF16 基准（默认，无量化）
#   bash 03_convert_llm_x86.sh bf16         # 同上
#   bash 03_convert_llm_x86.sh fp8          # FP8 量化 + 导出
#   bash 03_convert_llm_x86.sh nvfp4        # NVFP4 量化 + 导出
#   bash 03_convert_llm_x86.sh all          # bf16 + fp8 + nvfp4 全部
#   FORCE=1 bash 03_convert_llm_x86.sh fp8  # 覆盖已有产物重新执行
#
# 注意:
